...
This mypackage **R** package is one that we will be modifying in Math 3190. It starts out with four functions: `add()`, `ggraph()`, `hello()`, `multiply()`, `divide()`, and `subtract()`. All of these are quite simple except the `ggraph()`, which depends on `ggplot2` and gives a quick way to create a scatter plot with two vectors. 
